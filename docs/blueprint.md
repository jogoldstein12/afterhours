...
An explicit pass-the-phone party game for adults, shipped as a Next.js web app
and as a single-file offline build.
...
The game exists twice and the two are maintained separately:

- The **Next.js app** in `src/` — the product, with the age gate, legal pages, and error boundaries.
- **`game.html`** — the whole game in one file, no server and no dependencies, generated by `npm run build:html` from `tools/standalone-template.html` plus the deck in `src/lib/prompts.ts`.

CI enforces that the deck baked into `game.html` matches `prompts.ts`. It cannot
enforce that the two engines behave the same, so any gameplay change has to be
